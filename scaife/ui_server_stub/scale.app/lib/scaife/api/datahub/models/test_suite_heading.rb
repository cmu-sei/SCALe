  ...
  class TestSuiteHeading
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Scaife::Api::Datahub::TestSuiteHeading` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Scaife::Api::Datahub::TestSuiteHeading`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @test_suite_id.nil?
        invalid_properties.push('invalid value for "test_suite_id", test_suite_id cannot be nil.')
      end
    ...
    def valid?
      return false if @test_suite_id.nil?
    ...
    def hash
      [test_suite_id, test_suite_name, test_suite_version, manifest_urls, uploader_id, uploader_organization_id, author_source, code_languages].hash
    end
