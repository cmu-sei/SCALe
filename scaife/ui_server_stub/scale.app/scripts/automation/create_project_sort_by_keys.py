...
def main():
    ...
    try:
        sess.event_project_create(
            name="dos2unix/rosecheckers sort project",
            description="test project with sort-by-keys automation",
            src_file=src_file,
            tools=tools,
            languages=[1, 2, 3]
        )
        sort_keys = ["Meta-Alert (m) ID ASC", "Flag ASC", "Severity ASC"]
        res = sess.event_sort_by_keys_submit(sort_keys_selected=sort_keys)
        print(res)
    except FetchError, e:
