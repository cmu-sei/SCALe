...
The file "scaife/ABOUT" provides product and version information for a particular SCAIFE system release. This is a release of code and a system that instantiates the SCAIFE API, e.g., a virtual machine for SCAIFE v 1.2.2. The SCAIFE system version is not necessarily the same as the API for a particular SCAIFE module. Each of the four non-SCALe SCAIFE modules has an API version that is specified in its swagger/swagger.yaml file in a line specifying "version", as in the following line:
version: "1.2.2"
...
SCAIFE can be installed as containers on Linux machines. So far, we have only tested installs on Linux machines,
...
1. In SCALe, the Uploads Tools page currently only uploads one tool, not all tools as it should. This bug will be fixed in a near-future version.

1. Some of the classifier performance metrics are calculated incorrectly, using labeled data both to create the classifier and to test it. This bug will be fixed in a near-future version.
...
1. For particular combinations of classifier schemes with Microjuliet project data, the confidence values do not display in SCALe.
...
1. There is currently an automated test failure for the python ``test_confidence`` test. This is currently being addressed internally and we expect to issue a bugfix for this test soon.

1. Confidence values do not get updated when a classifier based on one project is run on another project.

1. Once loaded to SCAIFE subsequent modifications on a project are not recognized when trying to run a classifier using that project.
